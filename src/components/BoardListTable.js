...
const BoardListTable = () => {
  ...
  const searchCondition = useSelector(
    (state) => state.boardSlice.searchCondition
  );
  ...
  const changePage = useCallback((event, value) => {
    dispatch(getBoards({
        searchCondition,
        searchKeyword,
  ...
  return (
    <>
      <Container maxWidth="xl">
        <TableContainer component={Paper} style={{ marginTop: "3%" }}>
          <Table>
            ...
            <TableBody>
              {boards.content &&
                boards.content.map((board, index) => (
                  <TableRow key={index}>
      ...
      <Container maxWidth='xl'
                 style={{marginTop: '1%', marginBotton: '1%', display: 'flex', justifyContent: 'center'}}>
