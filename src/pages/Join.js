...
const Join = () => {
    ...
    (e) => {
      ...
      if (e.target.name === "password") {
        if (e.target.value === joinForm.passwordCheck) {
          setPasswordChk(true);
          document.querySelector("#password-check-success").style.display =
            "block";
          ...
          document.querySelector("#password-check-success").style.display =
            "none";
          document.querySelector("#password-check-fail").style.display =
            "block";
        }
      }
  ...
  const nicknameCheck = useCallback(async () => {
    try {
      ...
      if (response.data.item.nicknameCheckMsg === "invalid nickname") {
        alert("중복된 닉네임입니다. 다른 닉네임 ㄱㄱ");
        document.querySelector("#nickname").focus();
        return;
      } else {
        if (
          window.confirm(
            `${joinForm.nickname}은 사용 가능한 닉네임입니다. ㄱㄱ?`
          )
        ) {
          document
            .querySelector("#nickname-check-btn")
  ...
  const handleJoin = useCallback(
    (e) => {
      ...
      if (!passwordValidate) {
        alert("비번 특문, 숫자, 영문 조합 9자리 이상으로 ㄱ");
        return;
      }
      ...
      <form onSubmit={handleJoin}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            ...
            >
              비밀번호는 특수문자, 영문자, 숫자 조합의 9자리 이상으로
              지정하세요.
            </Typography>
          </Grid>
          <Grid item xs={12}>
