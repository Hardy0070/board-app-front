...
const Post = () => {
    ...
    const imageLoader = (file) => {
        ...
        reader.onload = (e) => {
            ...
            if(file.name.toLowerCase().match(/(.*?)\.(jpg|png|jpeg|gif|svg|bmp)$/)) {
                ...
                // 프로젝트 내에 src와 public 폴더가 존재하는데 정적파일들을 src폴더나 public 폴더에 둘 다 위치를 시킬 수 있는데 가져오는 방식이 다르다.
    ...
    const handlePost = useCallback((event) => {
        ...
        // input type='file'의 내용은 제외되고 폼데이터가 생성 된다.
        ...
        // boardDto 하나 담고 파일을 따로 올려줄 것이다(?)
        const sendFormData = new FormData();
    
        sendFormData.append('boardDto', boardDto);
    
        Array.from(uploadFiles).forEach(file => sendFormData.append('uploadFiles', file)); // uploadFiles라는 키 값으로 파일을 하나씩 업로드 하겠다.
